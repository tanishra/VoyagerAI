"""Pipeline payload store — Redis-backed with bounded in-memory fallback.
...
- ``pipeline:payload:{id}`` — one-shot payloads, TTL 1h (consumed within
  ...
  pop).
- ``pipeline:thread:{tid}:{key}`` — per-thread state (latest comparison,
  last constraints) needed by refine_itinerary on later turns, TTL 30d.

Same graceful-degradation pattern as geocode_cache.py / threads.py:
Redis primary, bounded in-memory dict when Redis is unreachable.
"""
...
class PayloadStore:
    ...
    async def set_thread_state(self, thread_id: str, key: str, data: dict) -> None:
        ...
        if r is not None:
            try:
                await r.set(rkey, json.dumps(data), ex=_THREAD_STATE_TTL_S)
                return
            except (RedisError, RuntimeError) as exc:
