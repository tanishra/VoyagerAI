...
describe('SharePage', () => {
  ...
  it('renders full itinerary section when share is valid', async () => {
    mockGetShare.mockResolvedValueOnce({
      itinerary: {
        destination: 'Paris, France',
        total_days: 3,
  ...
  it('renders plan your own trip CTA when share is valid', async () => {
    mockGetShare.mockResolvedValueOnce({
      itinerary: {
        destination: 'Paris, France',
        total_days: 3,
    ...
    await waitFor(() => {
      expect(screen.getByText('Plan your own trip with VoyagerAI')).toBeInTheDocument();
    });
  });
